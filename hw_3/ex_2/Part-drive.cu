...
int mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    ...
    cudaMalloc(&Ex_flat_gpu , field_size * sizeof(FPfield));
    cudaMalloc(&Ey_flat_gpu , field_size * sizeof(FPfield));
    cudaMalloc(&Ez_flat_gpu , field_size * sizeof(FPfield));
    cudaMalloc(&Bxn_flat_gpu, field_size * sizeof(FPfield));
    cudaMalloc(&Byn_flat_gpu, field_size * sizeof(FPfield));
    cudaMalloc(&Bzn_flat_gpu, field_size * sizeof(FPfield));
    cudaMalloc(&XN_flat_gpu, grd_size * sizeof(FPfield));    
    cudaMalloc(&YN_flat_gpu, grd_size * sizeof(FPfield));
    cudaMalloc(&ZN_flat_gpu, grd_size * sizeof(FPfield));
    ...
    mover_PC_kernel<<<(nop+TPB-1)/TPB, TPB>>>(part_x_gpu  , part_y_gpu  , part_z_gpu  ,
    ...
    cudaFree(part_v_gpu);
    cudaFree(part_w_gpu);
    cudaFree(Ex_flat_gpu);
    cudaFree(Ey_flat_gpu);
    cudaFree(Ez_flat_gpu);
    cudaFree(Bxn_flat_gpu);
    cudaFree(Byn_flat_gpu);
    cudaFree(Bzn_flat_gpu);
    cudaFree(XN_flat_gpu);
    cudaFree(YN_flat_gpu);
    cudaFree(ZN_flat_gpu);
